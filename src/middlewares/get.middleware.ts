...
export const getAllTabla = (tabla: string) => async (req: Request, res: Response) => {
    try {
...
export const getFilterTabla = (tabla: string) => async (req: Request, res: Response): Promise<Response> => {
    try {
        const conn = await connect();
        try {
...
export const deleteRow = (tabla: string) => async (req: Request, res: Response) => {
    try {
        const conn = await connect();
        try {
            const result = await conn.query('delete from ' + tabla + ' where ?', req.params);
...
export const InsertRow = (tabla: string) => async (req: Request, res: Response) => {
    try {
        const conn = await connect();
        try {
            const resultado = await conn.query("INSERT INTO " + tabla + " SET ?  ", req.body);
...
export const UpdateRow = (tabla: string) => async (req: Request, res: Response) => {
    try {
        const conn = await connect();
        try {
            const result = await conn.query('update ' + tabla + ' set ? where ?', [req.body, req.params]);
